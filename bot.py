...
async def announce(message: str):
    ...
    for chat_id in chats:
        try:
            ...
            pass
    print(f"[ANNOUNCE] {message} → terkirim ke {success} chat")

# === COMMANDS ===
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    owner_id = get_config("owner_id")
    if not owner_id:
        await update.message.reply_text("🔐 Bot belum dikonfigurasi.\nGunakan: /setowner [ID_ANDA]")
        return
...
async def shutdown():
    if _bot_instance:
        await announce("🔴 BOT BANDING OFF")
...
if __name__ == "__main__":
    ...
    if not token:
        raise ValueError("❌ BOT_TOKEN tidak ditemukan! Setel di Secrets (Replit) atau environment (Linux).")
    ...
    # Kirim pesan ON setelah bot siap
    async def send_online():
        await asyncio.sleep(3)
        await announce("🟢 BOT BANDING ON")

    print("\n🚀 Bot aktif!")
    asyncio.run(send_online())
    app.run_polling()